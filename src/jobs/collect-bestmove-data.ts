/**
 * Collect per-phase best move rates from existing DB players.
 *
 * Reads phase-accuracy-data.json (349 players already in DB with evals),
 * runs computeArenaStats() for each, and records phaseBestMoveRate.
 * No Stockfish or API calls needed — pure DB reads.
 *
 * Usage: npx ts-node src/jobs/collect-bestmove-data.ts
 */
...
const OUTPUT_FILE = path.join(__dirname, "../../bestmove-rate-data.json");
...
async function main() {
  ...
  const results: BestMoveDataPoint[] = [];
  let skipped = 0;
  ...
  for (let i = 0; i < players.length; i++) {
    ...
    const point: BestMoveDataPoint = {
      username,
      rating,
      bracket,
      opening: stats.phaseBestMoveRate.opening,
      middlegame: stats.phaseBestMoveRate.middlegame,
      endgame: stats.phaseBestMoveRate.endgame,
    };

    results.push(point);
    console.log(
      `[${i + 1}/${players.length}] ${username} (${rating}, ${bracket}): ` +
      `OPN=${point.opening?.toFixed(1) ?? "—"} MID=${point.middlegame?.toFixed(1) ?? "—"} ` +
      `END=${point.endgame?.toFixed(1) ?? "—"}`
    );
  }

  // Write output
  fs.writeFileSync(OUTPUT_FILE, JSON.stringify(results, null, 2));
  console.log(`\nDone. ${results.length} players written to bestmove-rate-data.json (${skipped} skipped).`);
